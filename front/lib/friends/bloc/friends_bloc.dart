...
class FriendsBloc extends Bloc<FriendsEvent, FriendsState> {
  ...
  Future<void> _onLoadFriends(LoadFriends event, Emitter<FriendsState> emit) async {
    ...
    try {
      ...
      friends.sort((a, b) => a.name.compareTo(b.name)); // Trie par ordre alphabétique par défaut
      emit(FriendsLoaded(friends: friends, isSorted: true)); // Indique que la liste est triée par ordre alphabétique
    } catch (e) {
    ...
    if (state is FriendsLoaded) {
      ...
      try {
        ...
        final updatedFriends = currentState.friends.map((friend) {
          return friend.id == event.friend.id ? friend.copyWith(isFavorite: !friend.isFavorite) : friend;
        }).toList();
        emit(FriendsLoaded(friends: updatedFriends, isSorted: currentState.isSorted));
      } catch (e) {
  ...
  Future<void> _onDeleteFriend(DeleteFriend event, Emitter<FriendsState> emit) async {
    if (state is FriendsLoaded) {
      final currentState = state as FriendsLoaded;
      try {
        await friendService.deleteFriend(event.userId, event.friend.id);
        final updatedFriends = currentState.friends.where((friend) => friend.id != event.friend.id).toList();
        emit(FriendsLoaded(friends: updatedFriends, isSorted: currentState.isSorted));
      } catch (e) {
  ...
  void _onSortFriends(SortFriends event, Emitter<FriendsState> emit) {
    if (state is FriendsLoaded) {
      ...
      sortedFriends.sort((a, b) {
        return currentState.isSorted ? b.name.compareTo(a.name) : a.name.compareTo(b.name);
      });
      emit(FriendsLoaded(friends: sortedFriends, isSorted: !currentState.isSorted));
    }
  }
}